-- Fix autocomplete: dedup by name only (not name+producer) so results are diverse
-- "barolo" should show Barolo, Barolo Riserva, Barolo Bussia etc, not 20x Barolo from different producers
...
begin
  ...
  if array_length(words, 1) >= 2 and length(w2) >= 2 then
    -- Multi-word: each word must match in name or producer
    return query
    with matches as (
    ...
    -- 3-char single word: fast prefix with name-only dedup
    return query
    with prefixed as (
    ...
    -- 4+ char single word: substring match with tiered ranking + name-only dedup
    return query
    with scored as (
      select w.id, w.name, w.producer, w.vintage, w.image_url,
        greatest(
